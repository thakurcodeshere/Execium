...
const PROVIDERS = [
  ...
  { id: "devto",      label: "Dev.to Community",             icon: "💻", color: "#a855f7", bg: "rgba(168,85,247,.08)", border: "rgba(168,85,247,.25)" },
...
export default function LoginPage() {
  ...
  const handleLogin = (provider: string) => {
    ...
    setTimeout(() => {
      localStorage.setItem("execium_user", JSON.stringify({ name, provider, ts: Date.now() }));
      router.push("/");
    }, 1400);
  };
  ...
  return (
    <div style={{
      ...
      {/* Glow orbs */}
      <div style={{ position: "fixed", top: "15%", left: "10%", width: 400, height: 400, borderRadius: "50%", background: "radial-gradient(circle,rgba(168,85,247,.12),transparent 70%)", pointerEvents: "none" }} />
      <div style={{ position: "fixed", bottom: "15%", right: "10%", width: 350, height: 350, borderRadius: "50%", background: "radial-gradient(circle,rgba(59,130,246,.1),transparent 70%)", pointerEvents: "none" }} />

      <div style={{
        width: "100%", maxWidth: 460, padding: "0 20px",
        animation: "slide-in-up .45s ease both",
      }}>
        {/* Card */}
        <div style={{
          ...
          boxShadow: "0 0 80px rgba(168,85,247,.12), 0 40px 80px rgba(0,0,0,.8)",
        }}>
          {alreadyIn ? (
            <div style={{ textAlign: "center" }}>
              <div style={{ fontSize: 44, marginBottom: 16 }}>✅</div>
              <div style={{ fontSize: 20, fontWeight: 800, color: "#10b981", marginBottom: 8 }}>You&apos;re signed in!</div>
              ...
              }}>Sign out and switch account</button>
            </div>
          ) : (
            <>
              {/* Header */}
              <div style={{ textAlign: "center", marginBottom: 36 }}>
                ...
                <h1 style={{ fontSize: 26, fontWeight: 900, color: "#e2e8f0", marginBottom: 8, letterSpacing: "-0.5px" }}>
                  Welcome to Execium
                </h1>
                <p style={{ fontSize: 13, color: "#475569" }}>
                  Join <span style={{ color: "#a855f7", fontWeight: 700 }}>50,000+ engineers</span> who think in execution.
                </p>
              </div>
                ...
                {PROVIDERS.map(p => (
                  <button
                    ...
                    style={{
                      ...
                      background: loading === p.id ? p.bg.replace(".08", ".15") : p.bg,
                      cursor: loading ? "not-allowed" : "pointer",
                      transition: "all .18s", opacity: loading && loading !== p.id ? .5 : 1,
                    }}
                  ...
                  >
                    ...
                    <span style={{ fontSize: 13, fontWeight: 600, color: p.color, flex: 1, textAlign: "left" }}>
              ...
              <div style={{
                ...
                textAlign: "center", fontSize: 11, color: "#334155", lineHeight: 1.8,
              }}>
                By signing in you agree to our{" "}
                <span style={{ color: "#a855f7", cursor: "pointer" }}>Terms of Service</span> and{" "}
                <span style={{ color: "#a855f7", cursor: "pointer" }}>Privacy Policy</span>.
                <br />No spam. No BS. Just code.
              </div>
            </>
          )}
        </div>
        ...
        {/* Tagline below card */}
        <p style={{
          textAlign: "center", marginTop: 24, fontSize: 12, color: "#1e293b",
