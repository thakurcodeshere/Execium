...
export default function ThemeSwitcher() {
  ...
  const filtered = THEME_LIST.filter(t => !search || t.name.toLowerCase().includes(search.toLowerCase()));
    ...
    <div ref={ref} style={{ position: 'relative' }}>
      ...
      >
        ...
        <div style={{ textAlign: 'left' }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: theme.uiText, fontFamily: "'JetBrains Mono'" }}>{theme.name}</div>
          <div style={{ fontSize: 9, color: theme.uiTextMuted, fontFamily: "'JetBrains Mono'", textTransform: 'uppercase', letterSpacing: 1 }}>
            {theme.type} · VS Code Theme
          </div>
        </div>
        ...
        <div style={{
          ...
          <div style={{ padding: '10px 12px', borderBottom: `1px solid ${theme.uiBorder}`, background: theme.uiPanelHd }}>
            ...
            <input autoFocus value={search} onChange={e => setSearch(e.target.value)} placeholder="Search themes…"
              style={{ width: '100%', padding: '6px 10px', borderRadius: 6, outline: 'none',
          ...
          <div style={{ maxHeight: 380, overflowY: 'auto', padding: '6px 0' }}>
            {GROUPS.map(g => {
              ...
              return (
                <div key={g.label}>
                  <div style={{ padding: '5px 14px 3px', fontSize: 9, fontWeight: 800, letterSpacing: 1.5,
                    textTransform: 'uppercase', color: theme.uiTextMuted, fontFamily: "'JetBrains Mono'" }}>{g.label}</div>
                  {items.map(t => (
                    <button key={t.id} onClick={() => { setTheme(t.id); setOpen(false); }}
                      style={{ width: '100%', padding: '7px 14px', display: 'flex', alignItems: 'center',
                      ...
                      <div>
                        <div style={{ fontSize: 12, fontWeight: t.id === theme.id ? 700 : 400,
                          color: t.id === theme.id ? theme.uiAccent : theme.uiText }}>{t.name}</div>
                        <div style={{ fontSize: 9, color: theme.uiTextMuted, fontFamily: "'JetBrains Mono'" }}>{t.type}</div>
                      </div>
                      {t.id === theme.id && <span style={{ marginLeft: 'auto', color: theme.uiAccent, fontSize: 12 }}>✓</span>}
                    </button>
                  ))}
                </div>
              );
